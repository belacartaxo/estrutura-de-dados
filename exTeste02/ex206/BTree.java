...
public class BTree<T> {   
   ...
   public T path(String p){
      if (p.equals("R")) return root.getValue();
      return path(root, p);
   }

   public T path(BTNode<T> n, String p){
      if(p.equals("")) return n.getValue();
      if(p.charAt(0) == 'E') return path(n.getLeft(), p.substring(1));
      return path(n.getRight(), p.substring(1));
   }
