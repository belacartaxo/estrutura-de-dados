...
public class BSTree<T extends Comparable<? super T>> {   
   ...
   public boolean valid(BSTNode<T> node, T lowerBound, T upperBound) {
      if (node == null) return true;
      if ((lowerBound != null && node.getValue().compareTo(lowerBound) <= 0) || (upperBound != null && node.getValue().compareTo(upperBound) >= 0)) 
         return false;
      return valid(node.getLeft(), lowerBound, node.getValue()) && valid(node.getRight(), node.getValue(), upperBound);
   }
